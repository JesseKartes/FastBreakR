...
#' @return A tibble containing combined FanDuel data for all game IDs.
#' @export
nba_fanduel <- function(game_ids, batch_size = 100, pause_seconds = 15) {
...
#' @return A list containing the raw FanDuel data and column names.
fetch_fanduel_data <- function(game_id) {
  headers <- generate_headers_stats()

  all_data <- map_dfr(game_id, function(game) {
    url <- paste0("https://stats.nba.com/stats/infographicfanduelplayer/?gameId=", game)
